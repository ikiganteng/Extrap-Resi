...
$j = 0;
    while(true){
        $randomAWB = 'UM2'.rand(00000000,99999999).'US';
        $check = us($randomAWB);
        $json_check = json_decode($check,true);
            if ($json_check['rajaongkir']['status']['code'] == 200) {
                        ...
                        file_put_contents('us.txt',"$awb : $status : $date : $origin : $destination ".PHP_EOL,FILE_APPEND);
                        $i++;
...
function us($awb){
    $url = 'https://www.cekpengiriman.com/wp-content/themes/resiongkir/data/awb/usps.php';
	$headers = array();
	...
	$body = 'nomor='.$awb.'&kurir=usps&type=awb&uuid=d06e0a32a8cec70eb9c59b29273c5c9b';
